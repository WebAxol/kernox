...
2. **Earlier parents (lower index) take precedence** over later parents — `inherits: [A, B]` means A's attributes win over B's when both define the same key
...
console.log(entity2.position.x); // Still 0 — separate copy
```
...
```typescript
// Return entity to pool — removes it from all collections
